...
const LandingPage = () => {
  ...
  const handleCardHover = (e) => {
    gsap.to(e.currentTarget, {
      y: -8,
      boxShadow: '0 20px 40px rgba(255, 161, 22, 0.2)',
      duration: 0.3,
      ease: 'power2.out',
    });
  };
  ...
  const handleCardHoverOut = (e) => {
    gsap.to(e.currentTarget, { y: 0, boxShadow: '0 0', duration: 0.3 });
  };
    ...
    <div className="flex min-h-screen flex-col bg-dark text-text-main">
      ...
      <main className="flex-1 overflow-hidden">
          ...
          <div className="relative z-10 mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            <div className="space-y-8">
              <h1 className="max-w-3xl text-5xl font-bold tracking-tight text-text-main md:text-7xl">
                Turn LeetCode progress into a <span className="bg-linear-to-r from-orange to-yellow bg-clip-text text-transparent">motivating streak engine</span>
              </h1>

              <p className="max-w-2xl text-lg leading-8 text-text-muted md:text-xl">
                LeetQuest turns your public LeetCode activity into XP, streaks, badges, analytics, and friendly competition.
              </p>

              <div className="flex flex-wrap gap-4">
                <Link to="/register" className="inline-flex items-center justify-center rounded-md bg-orange px-8 py-3 font-semibold text-dark hover:bg-orange-hover transition-all hover:scale-105">
                  Start your quest
                </Link>
                <a href="#features" className="inline-flex items-center justify-center rounded-md bg-light-gray px-8 py-3 font-medium text-text-main hover:bg-border transition-all hover:scale-105">
                  Explore features
                </a>
              </div>
              ...
              <div ref={statsRef} className="grid gap-4 sm:grid-cols-3">
                {quickStats.map((stat) => (
                  <div key={stat.label} className="rounded-2xl border border-border bg-dark-gray p-4 cursor-pointer" onMouseEnter={handleCardHover} onMouseLeave={handleCardHoverOut}>
            ...
            <div className="relative hover:scale-[1.02] transition-transform cursor-pointer">
                ...
                <div className="space-y-5">
                  <div className="flex items-center justify-between border-b border-border pb-4">
                    <div>
                      ...
                      <h2 className="mt-2 text-2xl font-semibold text-text-main">Your progress at a glance</h2>
                    </div>
                    <div className="rounded-full border border-green/30 bg-green/10 px-3 py-1 text-sm text-green">Synced today</div>
                  </div>

                  <div className="grid gap-4 md:grid-cols-2">
                    <div className="rounded-2xl border border-border bg-dark p-4 cursor-pointer hover:-translate-y-2 transition-transform">
                      <p className="text-sm text-text-muted">Streak</p>
                      <div className="mt-3 flex items-end justify-between">
                        <p className="text-5xl font-bold text-yellow">12</p>
                        <span className="rounded-full border border-border bg-light-gray px-3 py-2 text-sm font-semibold text-text-main">ST</span>
                      </div>
                    ...
                    <div className="rounded-2xl border border-border bg-dark p-4 cursor-pointer hover:-translate-y-2 transition-transform">
                      ...
                      <div className="mt-3 flex items-end justify-between">
                        ...
                        <span className="rounded-full border border-border bg-light-gray px-3 py-2 text-sm font-semibold text-text-main">XP</span>
                      </div>
                  ...
                  <div className="rounded-2xl border border-border bg-dark p-4">
                    ...
                    <div className="mt-4 space-y-3">
                      {[['Dynamic Programming', 'High priority'], ['Graphs', 'Needs practice'], ['Binary Trees', 'Steady']].map(([label, note]) => (
                        <div key={label} className="flex items-center justify-between rounded-xl bg-light-gray px-4 py-3 hover:bg-border transition-colors cursor-pointer">
        ...
        <section id="features" className="border-t border-border bg-dark-gray px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="mb-8 max-w-3xl space-y-3">
              ...
              <h2 className="text-3xl font-bold text-text-main md:text-4xl">Everything you need to stay consistent.</h2>
              <p className="text-text-muted">The app is intentionally narrow: it tracks progress, shows useful trends, and makes practice feel rewarding.</p>
            </div>
          ...
          <div className="mx-auto max-w-7xl rounded-4xl border border-border bg-dark-gray p-8 md:p-10">
            <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
              <div className="space-y-4">
                <p className="text-sm uppercase tracking-[0.3em] text-text-muted">How it works</p>
                <h2 className="text-3xl font-bold text-text-main md:text-4xl">A simple flow with no extra friction.</h2>
                <p className="text-text-muted">Connect once, sync public data, and use the dashboards to keep yourself on track.</p>
              </div>

              <div ref={workflowRef} className="grid gap-4">
                {workflowSteps.map((step) => (
                  <div key={step.step} className="rounded-2xl border border-border bg-dark p-5 cursor-pointer hover:translate-x-2 transition-transform">
                    <div className="flex items-start gap-4">
                      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-orange text-sm font-bold text-dark">{step.step}</div>
                      <div>
        ...
        <section className="px-4 pb-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl rounded-4xl border border-orange/20 bg-linear-to-r from-dark-gray to-dark p-8 text-center md:p-12 cursor-pointer hover:shadow-lg transition-all" style={{ boxShadow: '0 0 0px rgba(255, 161, 22, 0)' }} onMouseEnter={(e) => { e.currentTarget.style.boxShadow = '0 0 60px rgba(255, 161, 22, 0.2)'; }} onMouseLeave={(e) => { e.currentTarget.style.boxShadow = '0 0 0px rgba(255, 161, 22, 0)'; }}>
            <p className="text-sm uppercase tracking-[0.3em] text-text-muted">Ready to start</p>
            <h2 className="mt-4 text-3xl font-bold text-text-main md:text-5xl">Build momentum before your next interview.</h2>
            <p className="mx-auto mt-4 max-w-2xl text-text-muted">Create your account, connect your LeetCode username, and let the platform turn daily practice into visible progress.</p>
            <div className="mt-8 flex flex-wrap justify-center gap-4">
              <Link to="/register" className="inline-flex items-center justify-center rounded-md bg-orange px-8 py-3 font-semibold text-dark hover:bg-orange-hover transition-all hover:scale-105">
                Create account
              </Link>
              <Link to="/login" className="inline-flex items-center justify-center rounded-md border border-border px-8 py-3 font-medium text-text-main hover:bg-light-gray transition-all hover:scale-105">
                Log in
              </Link>
            </div>
          </div>
